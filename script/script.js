...
class Heroes {
  ...
  getUniverse() {
    return "Вселенная: " + this.universe;
  }
  getAlterego() {
    return "Альтер эго: " + this.alterego;
  }
  getOccupation() {
    return "Род деятельности: " + this.occupation;
  }
  getFriends() {
    return "Друзья: " + this.friends;
  }
  getSuperpowers() {
    return "Суперсилы: " + this.superpowers;
  }
  getName() {
    return this.name;
  }
...
data.forEach((hero) => {
  ...
  divHeroes.append(divHero);

  // Информация о герое
  let heroInfo = document.createElement("div");
  heroInfo.className = "info";

  // Кнопка
  let heroButton = document.createElement("button");
  divHero.append(heroButton);
  heroButton.append("Click");
  heroButton.addEventListener("click", function () {
    heroButton.append(heroInfo);
    heroInfo.textContent = hero.info;
    heroInfo.classList.toggle("hide");
  });

  // Имя героя
  let heroTitle = document.createElement("h2");
  divHero.append(heroTitle);
  heroTitle.append(hero.name);
  // Вселенная героя
  let heroUniverse = document.createElement("p");
  divHero.append(heroUniverse);
  heroUniverse.append(dataClass.getUniverse());
  // Альтер эго героя
  let heroAlterego = document.createElement("p");
  divHero.append(heroAlterego);
  heroAlterego.append(dataClass.getAlterego());
  // Род деятельности героя
  let heroOccupation = document.createElement("p");
  divHero.append(heroOccupation);
  heroOccupation.append(dataClass.getOccupation());
  // Друзья героя
  let heroFriends = document.createElement("p");
  divHero.append(heroFriends);
  heroFriends.append(dataClass.getFriends());
  // Суперсилы героя
  let heroSuperpowers = document.createElement("p");
  divHero.append(heroSuperpowers);
  heroSuperpowers.append(dataClass.getSuperpowers());
  // Фото героя
  let heroImg = document.createElement("img");
  heroImg.setAttribute("src", hero.url);
  divHero.append(heroImg);
  // Звездный рейтинг
  let divRating = document.createElement("div");
  divRating.className = "rating";
  divHero.append(divRating);
  rating.forEach((star) => {
    let starInput = document.createElement("input");
    starInput.setAttribute("type", "radio");
    starInput.setAttribute("name", "star");
    starInput.setAttribute("id", `${dataClass.getName()}_${star.id}`);
    divRating.append(starInput);
    let starLabel = document.createElement("label");
    starLabel.setAttribute("for", `${dataClass.getName()}_${star.id}`);
    starLabel.setAttribute("id", "star");
    starLabel.className = `${dataClass.getName()}_${star.id}`;
    divRating.append(starLabel);
    starInput.addEventListener("change", function () {
      if (starInput.checked) {
        window.localStorage.setItem(dataClass.getName(), star.grade);
      }
    });
    if (
      window.localStorage.getItem(dataClass.getName()) == 1 &&
      starLabel.classList == `${dataClass.getName()}_star_1`
    ) {
      starLabel.style.color = "gold";
    } else if (
      (window.localStorage.getItem(dataClass.getName()) == 2 &&
        starLabel.classList == `${dataClass.getName()}_star_1`) ||
      (window.localStorage.getItem(dataClass.getName()) == 2 &&
        starLabel.classList == `${dataClass.getName()}_star_2`)
    ) {
      starLabel.style.color = "gold";
    } else if (
      (window.localStorage.getItem(dataClass.getName()) == 3 &&
        starLabel.classList == `${dataClass.getName()}_star_1`) ||
      (window.localStorage.getItem(dataClass.getName()) == 3 &&
        starLabel.classList == `${dataClass.getName()}_star_2`) ||
      (window.localStorage.getItem(dataClass.getName()) == 3 &&
        starLabel.classList == `${dataClass.getName()}_star_3`)
    ) {
      starLabel.style.color = "gold";
    } else if (
      (window.localStorage.getItem(dataClass.getName()) == 4 &&
        starLabel.classList == `${dataClass.getName()}_star_1`) ||
      (window.localStorage.getItem(dataClass.getName()) == 4 &&
        starLabel.classList == `${dataClass.getName()}_star_2`) ||
      (window.localStorage.getItem(dataClass.getName()) == 4 &&
        starLabel.classList == `${dataClass.getName()}_star_3`) ||
      (window.localStorage.getItem(dataClass.getName()) == 4 &&
        starLabel.classList == `${dataClass.getName()}_star_4`)
    ) {
      starLabel.style.color = "gold";
    } else if (
      (window.localStorage.getItem(dataClass.getName()) == 5 &&
        starLabel.classList == `${dataClass.getName()}_star_1`) ||
      (window.localStorage.getItem(dataClass.getName()) == 5 &&
        starLabel.classList == `${dataClass.getName()}_star_2`) ||
      (window.localStorage.getItem(dataClass.getName()) == 5 &&
      ...
      (window.localStorage.getItem(dataClass.getName()) == 5 &&
        starLabel.classList == `${dataClass.getName()}_star_4`) ||
      (window.localStorage.getItem(dataClass.getName()) == 5 &&
        starLabel.classList == `${dataClass.getName()}_star_5`)
    ) {
      starLabel.style.color = "gold";
    }
  });
